...
from workflow.video.multi_channel import multi_channel_run
from workflow.video.rtsp.one_channel import RtspVideoWorkflowOneChannel


@app.command(help='视频监控场景测试 - 文件存储: 流直存', hidden=False)
def video_rtsp(
        # 存储 环境信息
        target: str = typer.Option(..., help="环境信息：例 /mnt/test or D:\\test"),
        rtsp_url: str = typer.Option(..., '-f', help="rtsp配置文件"),
        ...
        process_workers: int = typer.Option(1, min=1, help="自定义：多进程运行协程，进程数"),
    ...
    init_logger(prefix='video_rtsp', case_id=case_id, trace=trace)
    if not appendable:
        # 非追加写模式，分片=1
        segments = 1
    ...
    multi_channel_run(
        RtspVideoWorkflowOneChannel, channel_num, process_workers,
        rtsp_url=rtsp_url,
        target=target, vs_info=vs_info,
