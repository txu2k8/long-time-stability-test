...
from core.video_monitor.video_2 import VideoMonitor2
...
@app.command(help='视频监控场景测试-1，读取数据库，写删不同协程中并行处理，多对象并行处理')
def video_monitor_1(
...
@app.command(help='视频监控场景测试-2，不读取数据库，写删不同协程中并行处理，多对象并行处理')
def video_monitor_2(
        endpoint: str = typer.Option(..., help="例：127.0.0.1:9000 or http://127.0.0.1:9000"),
        access_key: str = typer.Option(..., help="ACCESS_KEY"),
        secret_key: str = typer.Option(..., help="SECRET_KEY"),
